...
**Action**: Verify required MCP tools are available before running analysis
**Tools**: Azure MCP + GitHub MCP
**Process**:
1. Confirm Azure MCP tools are available for discovery/monitoring (for example, `azmcp-subscription-list`, `azmcp-group-list`, `azmcp-bestpractices-get`).
2. Confirm GitHub MCP issue creation tool is available (`create_issue`).
3. If any required tool is missing, stop and prompt installation/configuration before continuing:

```text
I cannot run Azure cost optimization yet because required tools are missing:
- [missing tool names]

Please install/configure and authenticate the missing MCP server(s):
1) Azure MCP server (for Azure resource discovery and monitoring)
2) GitHub MCP server (for issue creation)

After setup is complete, tell me to continue and I will resume from Step 1.
```
...
- **MCP Tool Missing**: Stop and provide install/configuration instructions for missing Azure MCP or GitHub MCP tools, then resume only after user confirms setup
